...
- Если нужно понять, почему work item нельзя claim/apply прямо сейчас, используй `madspec memory coordinator explain --work-item-id ...` или `--session-key ...` вместо догадок по косвенным признакам.
