...
MADSpec вводит модель памяти как архитектуру хранения и извлечения контекста, а не как отдельный формат файлов. Причина в том, что для длинной работы с ветками, несколькими сессиями и параллельными агентами недостаточно набора удобочитаемых Markdown-артефактов: они хороши как представление, но плохо подходят на роль главного источника истины, если нужно последовательно фиксировать состояние, объяснять выбор следующего шага и безопасно восстанавливать проекции после сбоев.

Поэтому память разделена на два слоя:

- канонический слой отвечает за действительное состояние процесса и данных;
- семантический слой отвечает за поиск релевантного контекста по смыслу поверх канонических данных.

### Канонический Слой

Канонический слой — это источник истины для записей, снимков стадий, состояния сессий, runtime-ревизий и координационных сущностей. Его основой служит `.madspec/system/memory/memory.sqlite`.

Этот слой нужен, чтобы:

- фиксировать подтвержденное состояние ветки и проекта в форме, пригодной для согласованных изменений;
- отделять запись истины от пересобираемых представлений;
- давать командам объяснения и диагностики опору на одно и то же состояние.

Файлы в `.madspec/<branch>/memory/`, веточные `json/jsonl` и производные Markdown-артефакты остаются проекциями: они читаемы и удобны, но пересобираются из канонического состояния и не заменяют его.

### Семантический Слой

Семантический слой не хранит истину отдельно от канонического состояния. Его задача — помогать `retrieve` и `search` находить релевантные записи по смыслу. Для этого MADSpec использует векторный индекс в `.madspec/system/memory/lancedb/` и конфигурацию `memory.embeddings` в `.madspec/config.json`.

Семантический слой нужен, когда:

- формулировка запроса не совпадает буквально с текстом записи;
- агенту нужно быстро поднять похожие решения, факты или контракты;
- одного точного и полнотекстового совпадения недостаточно для хорошего контекста.

Важно не путать:

- модель памяти MADSpec — это вся архитектура памяти;
- канонический слой — это источник истины;
- локальная embedding-модель — это только инструмент семантического поиска;
- векторный индекс — это не отдельная каноническая база знаний, а производный слой чтения поверх канонических записей.

### Как Слои Работают Вместе

Обычный рабочий цикл выглядит так:

1. `capture`, `checkpoint`, `register-step`, `start-step`, `checkpoint-step`, `complete-step` и связанные команды сначала фиксируют каноническое состояние.
2. После записи пересобираются веточные проекции и Markdown-представления.
3. Активное пространство индекса использует эти канонические данные для семантического поиска.
4. `retrieve` и `search` собирают ответ из точного, полнотекстового и семантического каналов.
5. `doctor`, `status` и `db-status` помогают понять, в каком именно слое возникла проблема.

Из этого следует важное различие:

- `madspec memory bootstrap-model` готовит локальный кэш embedding-модели;
- `madspec memory reindex` пересобирает активное пространство индекса для текущей конфигурации;
- успешный bootstrap модели сам по себе не означает, что активный индекс уже подтвержден и готов к использованию для новой конфигурации.
