...
- [ ] Все зависимости уже запланированы в `.madspec/memory/progress.json`
...
- [ ] Функции явно указаны в `coversFunctions` в `.madspec/memory/progress.json`

## Создание файлов шага

### 3. Создание директории шага
- [ ] Директория создана в формате `.madspec/steps/step-[NN]-[name]/`
...
- [ ] **ИСПОЛЬЗОВАН ШАБЛОН**: `.madspec/templates/step-validation-template.md` как основа
...
**Решение**: 
- Использовать шаблон `.madspec/templates/step-validation-template.md`
...
**Решение**: Проверить `plannedSteps.length` в `.madspec/memory/progress.json` и использовать `NN = plannedSteps.length + 1`
