...
- [ ] Все зависимости уже запланированы в `.madspec/<BRANCH>/memory/progress.json`
...
- [ ] Директория создана в формате `.madspec/<BRANCH>/steps/step-[NN]-[name]/`
...
**Решение**: Проверить `plannedSteps.length` в `.madspec/<BRANCH>/memory/progress.json` и использовать `NN = plannedSteps.length + 1`
