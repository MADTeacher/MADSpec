...
На этом этапе **MADSpec (MADSpec Framework)** создается визуальный дизайн пользовательского интерфейса на основе концепции проекта. Это критически важный этап, так как дизайн определяет:
...
- Должен существовать `.madspec/concept.md` и `.madspec/project-context.md` с описанной концепцией проекта
- Если файлы отсутствуют, предложи сначала выполнить `/madspec.concept`
...
2. **Проектирование интерфейса**:
   ...
   a. **Определение экранов**:
    ...
    b. **Создание HTML/CSS прототипов**:
       - **Шаблоны для прототипов**:
         - Используй шаблон `.madspec/templates/html-prototype-template.html` для создания отдельных экранов. **ВАЖНО**: Это голый каркас без предопределённых компонентов. Создай компоненты интерфейса СПЕЦИФИЧНЫЕ для домена проекта на основе concept.md. Добавь необходимые стили в секцию `<style>` каждого экрана.
       ...
       - **Группировка экранов по функциональным зонам**:
         - На основе `.madspec/concept.md` определи функциональные зоны приложения
   ...
   e. **Создание локального сервера**
      ...
      - Создай проект локального сервера в `.madspec/ui-prototype/` , который будет возвращать `index.html` (например, http://localhost:8000/index.html)
      - Создай в директории `.madspec/ui-prototype/` файл `README.md` с описанием того, как запустить локальный сервер и открыть главную страницу
...
5. **Создание файлов дизайна**:
   - Создай директорию `.madspec/ui-prototype/` для HTML/CSS прототипов
   ...
   - Сохрани описание дизайна в `.madspec/ui-design.md`
...
6. **Требование для утверждения дизайна**:
   ...
   - Укажи путь к главному файлу прототипа: `.madspec/ui-prototype/index.html`
...
7. **Отчет**:
   - Выведи путь к главному файлу прототипа: `.madspec/ui-prototype/index.html`
...
## Правила
- **НЕ ИСПОЛЬЗУЙ предопределённые компоненты** из шаблонов (карточки, формы, списки, кнопки). Шаблоны — это голый каркас. **Создавай компоненты, специфичные для домена проекта** на основе `.madspec/concept.md`.
...
- `.madspec/templates/html-prototype-template.html` - шаблон для отдельных экранов
- `.madspec/templates/index-prototype-template.html` - шаблон для главной страницы с навигацией
...
- `.madspec/ui-design.md` - описание дизайна интерфейса (ссылки на прототипы, описание потоков)
- `.madspec/project-context.md` - обновленный контекст проекта

## Следующий этап

После утверждения дизайна перейди к `/madspec.tech` для выбора технологического стека.
