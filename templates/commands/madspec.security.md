...
Эта команда **MADSpec (MADSpec Framework)** выполняет комплексную проверку безопасности кода проекта на соответствие стандартам OWASP и законодательству по персональным данным. Команда поддерживает настраиваемый scope проверки для разных уровней сложности и задач проекта.

## Предварительные условия

- **Должен существовать** реализованный код проекта (хотя бы частично)
- **Должен существовать** `.madspec/<BRANCH>/tech-stack.md` для понимания технологического стека (где `<BRANCH>` определяется через `madspec git current-branch`)
- **Должен существовать** `.madspec/<BRANCH>/architecture.md` для анализа архитектурных решений безопасности
- Если кода нет, предложи выполнить этап реализации (`/madspec.implement`)
...
- `/madspec.security --scope full --jurisdiction gdpr` - полная проверка с GDPR
- `/madspec.security --skip-artifacts` - базовая проверка без обязательных артефактов
- `/madspec.security --scope code --skip-artifacts` - только анализ кода без артефактов
...
- Если кода нет и не указан `--skip-artifacts`, сообщи пользователю и предложи выполнить `/madspec.implement`
...
- Проверь существование `.madspec/<BRANCH>/tech-stack.md`
- Проверь существование `.madspec/<BRANCH>/architecture.md`
- Проверь существование `.madspec/<BRANCH>/concept.md`
- Проверь существование `.madspec/<BRANCH>/data-model.md` (опционально)
- Если какие-то артефакты отсутствуют:
  - Сообщи пользователю, какие артефакты не найдены
  - Предложи выполнить соответствующие команды MADSpec для их создания
  - Спроси, продолжать ли проверку с доступными артефактами или прервать

**Загрузка артефактов:**
- Прочитай `.madspec/<BRANCH>/tech-stack.md`, если он существует
- Прочитай `.madspec/<BRANCH>/architecture.md`, если он существует
- Прочитай `.madspec/<BRANCH>/concept.md`, если он существует
- Прочитай `.madspec/<BRANCH>/data-model.md`, если он существует
...
- Предложи пользователю установить и запустить найденный пакет для сканирования зависимостей
...
```
/madspec.security
/madspec.security --scope full
/madspec.security --scope owasp-top10
/madspec.security --scope asvs,privacy
/madspec.security --jurisdiction gdpr
/madspec.security --scope full --jurisdiction 152fz,gdpr
```
