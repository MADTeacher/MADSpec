...
- Желательно: `.madspec/architecture.md`
...
Создать `.madspec/deployment.md`, который отвечает на вопросы:
...
0. **Загрузка контекста**:
   - Прочитай `.madspec/concept.md`, `.madspec/tech-stack.md`, `.madspec/project-context.md`
   - Если существует — прочитай `.madspec/architecture.md`
   - Выдели компоненты системы, данные/чувствительность, интеграции, требования к доступности.

1. **Определение режима (early/retrofit)**:
   - Если уже существует `.madspec/implementation-plan.md` или `.madspec/steps/` или заметен реализованный код — работай в режиме **retrofit** (аудит + план доработок).
   - Иначе — режим **early**.
...
4. **Выбор подхода к хостингу**:
   ...
   - Выбери один вариант (по одному вопросу за раз).
...
9. **Безопасность деплоя**:
   ...
   - Если есть ПДн/секреты — свяжи с `/madspec.security`.

10. **Сохранение артефакта**:
   - Загрузи шаблон `.madspec/templates/deployment-template.md`
   - Заполни и сохрани как `.madspec/deployment.md`
   - Обнови `.madspec/project-context.md` (текущий этап = `deploy`, ссылка на `.madspec/deployment.md`)

11. **Если режим retrofit**:
   - Сделай gap-анализ: что нужно добавить/переделать для выбранного деплоя.
   - Сформируй список доработок и предложи включить их в план реализации (через `/madspec.plan`) или создать отдельные шаги.

## Выходные артефакты

- `.madspec/deployment.md`
- Обновленный `.madspec/project-context.md`
