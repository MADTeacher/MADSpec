...
3. **Определение review journeys**:
   ...
   - Для каждого flow определи входной экран, последовательность экранов и ожидаемый результат.
   - `flows[0]` считай primary flow, а его первый шаг — обязательной точкой входа приложения на `index.html`.
...
4. **Создание HTML/CSS UI-прототипов**:
   ...
   - `index.html` — это реальный входной экран приложения: auth, onboarding, setup или первый рабочий экран.
   - `Home` в review overlay всегда ведет на точку входа приложения `index.html`, а не на отдельный обзорный экран.
   - Review journeys и их описания фиксируются в памяти и `ui-design.md`; отдельная HTML-страница с картой journeys не обязательна.
...
5. **Фиксация flows и экранов в memory**:
   ...
   - Используй текущую модель `screens`, `flows`, `navigation` как canonical основу:
     ...
     - первый step flow = входной экран journey;
...
7. **Валидация дизайна**:
   ...
   - Проверь, что ровно один экран ссылается на `.madspec/<BRANCH>/ui-prototype/index.html`.
   - Проверь, что primary flow начинается именно с этого экрана и проходится кликами до финального экрана без ручного редактирования URL.
...
- **`index.html` — это точка входа приложения**, а не обзорная страница journeys.
...
- `.madspec/<BRANCH>/ui-prototype/` - директория с HTML/CSS UI-прототипами
  - `index.html` - реальная точка входа приложения для primary flow
