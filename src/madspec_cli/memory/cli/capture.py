...
CAPTURE_FROM_FILE_ALIASES = {
    ...
    "flow_alternative": "flow_alternatives",
    "nav": "navigation",
    "platform_constraint": "platform_constraints",
    "requirement": "requirements",
    "structure_note": "structure_notes",
    "preference": "preferences",
    "tech_constraint": "tech_constraints",
    ...
    "entity_state": "entity_states",
    "endpoint": "endpoints",
    "endpoint_screen": "endpoint_screens",
    "endpoint_field": "endpoint_fields",
    "endpoint_error": "endpoint_errors",
    "integration": "integrations",
    "code_principle": "code_principles",
    "pattern": "architecture_patterns",
    "security_note": "security_notes",
    "performance_note": "performance_notes",
    "planning_principle": "planning_principles",
    "deploy_overview": "deploy_overview",
    "deploy_goal": "deploy_goals",
    "environment": "environments",
    "deployment_unit": "deployment_units",
    "config_note": "config_notes",
    "secret_note": "secret_notes",
    "cicd_trigger": "cicd_triggers",
    "cicd_step": "cicd_steps",
    "release_artifact": "release_artifacts",
    "migration_note": "migration_notes",
    "backup_note": "backup_notes",
    "recovery_check": "recovery_checks",
    "observability_note": "observability_notes",
    "security_control": "security_controls",
    "release_strategy": "release_strategy",
    "rollback_strategy": "rollback_strategy",
    "existing_module": "existing_modules",
    "modified_file": "modified_files",
    "new_file": "new_files",
    "interface_contract": "interface_contracts",
    "dependency": "dependencies",
    "risk": "risks",
    "recommendation": "recommendations",
    "tech_note": "tech_notes",
    "architecture_note": "architecture_notes",
}
...
def memory_capture(
    ...
    stage: str = typer.Option(None, "--stage", help="Stage to capture: mvp.concept, mvp.design, mvp.tech, deploy, mvp.architecture, mvp.plan, feature.init, feature.plan, review, or security"),
    ...
    deploy_overview: str = typer.Option(None, "--deploy-overview", help="Deploy-only: краткое описание схемы развертывания"),
    deploy_goal: list[str] = typer.Option(None, "--deploy-goal", help="Deploy-only: цель развертывания; повторяйте параметр для нескольких значений"),
    environment: list[str] = typer.Option(None, "--environment", help="Deploy-only: окружение в формате '<name>::<purpose>::<notes>'; повторяйте параметр для нескольких значений"),
    deployment_unit: list[str] = typer.Option(None, "--deployment-unit", help="Deploy-only: единица развертывания в формате '<name>::<kind>::<runtime>::<notes>'; повторяйте параметр для нескольких значений"),
    config_note: list[str] = typer.Option(None, "--config-note", help="Deploy-only: заметка о конфигурации; повторяйте параметр для нескольких значений"),
    secret_note: list[str] = typer.Option(None, "--secret-note", help="Deploy-only: заметка о секретах; повторяйте параметр для нескольких значений"),
    cicd_trigger: list[str] = typer.Option(None, "--cicd-trigger", help="Deploy-only: триггер CI/CD; повторяйте параметр для нескольких значений"),
    cicd_step: list[str] = typer.Option(None, "--cicd-step", help="Deploy-only: шаг CI/CD; повторяйте параметр для нескольких значений"),
    release_artifact: list[str] = typer.Option(None, "--release-artifact", help="Deploy-only: артефакт релиза; повторяйте параметр для нескольких значений"),
    migration_note: list[str] = typer.Option(None, "--migration-note", help="Deploy-only: заметка о миграциях; повторяйте параметр для нескольких значений"),
    backup_note: list[str] = typer.Option(None, "--backup-note", help="Deploy-only: заметка о резервном копировании; повторяйте параметр для нескольких значений"),
    recovery_check: list[str] = typer.Option(None, "--recovery-check", help="Deploy-only: проверка восстановления; повторяйте параметр для нескольких значений"),
    observability_note: list[str] = typer.Option(None, "--observability-note", help="Deploy-only: заметка о наблюдаемости; повторяйте параметр для нескольких значений"),
    security_control: list[str] = typer.Option(None, "--security-control", help="Deploy-only: контроль безопасности; повторяйте параметр для нескольких значений"),
    release_strategy: str = typer.Option(None, "--release-strategy", help="Deploy-only: стратегия релиза"),
    rollback_strategy: str = typer.Option(None, "--rollback-strategy", help="Deploy-only: стратегия отката"),
    ...
    if from_file:
        file_data = read_args_file(
            from_file,
            aliases=CAPTURE_FROM_FILE_ALIASES,
        ...
        options = {
            ...
            "deploy_overview": deploy_overview,
            "deploy_goals": deploy_goal or [],
            "environments": environment or [],
            "deployment_units": deployment_unit or [],
            "config_notes": config_note or [],
            "secret_notes": secret_note or [],
            "cicd_triggers": cicd_trigger or [],
            "cicd_steps": cicd_step or [],
            "release_artifacts": release_artifact or [],
            "migration_notes": migration_note or [],
            "backup_notes": backup_note or [],
            "recovery_checks": recovery_check or [],
            "observability_notes": observability_note or [],
            "security_controls": security_control or [],
            "release_strategy": release_strategy,
            "rollback_strategy": rollback_strategy,
