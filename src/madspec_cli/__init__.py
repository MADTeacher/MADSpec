...
# ///
"""
MADSpec CLI - Setup tool for MADSpec projects

Usage:
    uvx madspec init <project-name>
    uvx madspec init .
    uvx madspec init --here

Or install globally:
    uv tool install --from madspec madspec
    madspec init <project-name>
    madspec init .
    madspec init --here
"""
...
def _resolve_branch_name(project_path: Path, branch_name: str | None) -> str:
    return branch_name or detect_branch(project_path)
...
def memory_promote(
    ...
    if json_output:
        _emit_json({"branch": target_branch, "promoted": payload})
        return
...
def memory_learn(
    ...
    if json_output:
        _emit_json({"branch": target_branch, **payload})
        return
...
def init(
    ...
    Examples:
        madspec init my-project
        madspec init my-project --ai cursor-agent
        madspec init my-project --ai opencode --no-git
        madspec init . --ai cursor-agent         # Initialize in current directory
        madspec init --here --ai sourcecraft    # Alternative syntax for current directory
        madspec init --here --force  # Skip confirmation when current directory not empty
    """
    ...
    with Live(
        ...
        try:
            ...
            try:
                ...
                ensure_memory_layout(project_path, current_branch)
                consolidate_branch_memory(project_path, current_branch)
                memory_errors = validate_branch_memory(project_path, current_branch)
                if memory_errors:
    ...
    enhancement_lines = [
        ...
        "",
        f"○ [cyan]/madspec.review[/] [bright_black](optional)[/bright_black] - Review and reflect on your implementation",
    ]
    ...
    safety_lines = [
        ...
        "",
        f"○ [cyan]/madspec.security[/] [bright_black](optional)[/bright_black] - Security audit (OWASP/privacy/dependencies/code)",
    ]
    safety_panel = Panel(
        "\n".join(safety_lines),
