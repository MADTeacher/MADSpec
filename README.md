...
- Локальное состояние выполнения для каждой сессии хранится в `SQLite` и выбирается через `--session-key`, при этом `active-session.json` сохраняется как производная проекция для совместимости с режимом одного агента
- Полный протокол параллельной работы нескольких агентов включен по умолчанию: новые проекты получают `parallelRuntime.phase2Enabled=true`, а `task/work-item/proposals/coordinator` доступны без дополнительного rollout-флага
- Ключ `parallelRuntime.phase2Enabled` позволяет явно отключить полный протокол `Phase 2`, если проекту временно нужен режим без coordinator-flow
...
- Для локальной семантической модели есть явный ручной путь подготовки локального кэша проекта: `madspec memory bootstrap-model`, после которого выполняется `madspec memory reindex`
...
Для легкой задачи или маленькой MVP-итерации агент должен предпочитать один полный шаг планирования, а не дробить работу на искусственные микро-шаги. Отдельные шаги нужны только там, где есть реальные зависимости, разные риски, самостоятельные точки проверки или явная просьба пользователя идти подробнее.
Приоритеты `P1/P2/P3` в концепции нужны для покрытия и очередности, но сами по себе не означают, что на каждую функцию нужен отдельный шаг плана.
...
Для состояния выполнения и его изменений MADSpec теперь использует каноническое хранилище в `SQLite`: `progress`, снимки стадий, состояние конкретной сессии и поток записей сначала фиксируются в `.madspec/system/memory/memory.sqlite`, а веточные `memory/*.json`, `memory/*.jsonl` и производные Markdown-представления затем пересобираются как проекции. Команды `madspec memory retrieve/search/capture/checkpoint/register-step/start-step/checkpoint-step/complete-step` и `madspec agents subagents context` принимают `--session-key`, а значение по умолчанию остается `active`, чтобы не ломать текущий поток работы одного агента.
...
- `Phase 1` с параллельными планированием и реализацией остается доступным и не отключается;
- полный протокол `Phase 2` для `task/work-item/proposals/coordinator` включен по умолчанию;
- `parallelRuntime.phase2Enabled=false` остается допустимым ручным переключателем, если нужно временно отключить coordinator-flow;
- `madspec migrate` нормализует конфигурацию к текущему проектному контракту и не требует отдельного rollout-шага для включения Phase 2.
...
В русскоязычных документах MADSpec повествовательный текст пишется по-русски. Английский допустим только для именованных технических сущностей: команд, путей, имен файлов, JSON-ключей, названий продуктов, протоколов, библиотек и других точных идентификаторов.
