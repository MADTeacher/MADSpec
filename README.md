...
- Полный протокол параллельной работы нескольких агентов включается явно: новые проекты получают `parallelRuntime.phase2Enabled=false` по умолчанию, а `task/work-item/proposals/coordinator` активируются через `.madspec/config.json`
- Ключ `parallelRuntime.phase2Enabled` определяет, остается ли проект в стандартном режиме `Phase 1` или явно включает полный протокол `Phase 2`
- Полный `Phase 2` сейчас работает как opt-in режим.
...
- `Phase 1` с параллельными планированием и реализацией считается стандартным режимом;
- полный протокол `Phase 2` для `task/work-item/proposals/coordinator` остается режимом с явным включением;
- старые проекты без этого блока продолжают работать в совместимом режиме без ручной миграции;
- `madspec migrate` меняет только структуру `.madspec/`, а не режим поведения проекта.
