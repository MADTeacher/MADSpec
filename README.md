...
1. **Начните с концепции**:
   ```
   /madspec.mvp.concept Описание вашей идеи проекта
   ```
   Агент создаст концепцию, инициализирует GIT репозиторий с `.gitignore`, создаст ветку для разработки и создаст первый коммит. Все артефакты сохраняются в `.madspec/<branch>/`.
...
1. **Инициализируйте работу над функциональностью**:
   ```
   /madspec.feature.init Описание новой функциональности
   ```
   Команда проанализирует существующий проект, создаст feature ветку и подготовит контекст для работы.

2. **Создайте план реализации** (инкрементально):
   ```
...
- **Проверка безопасности кода** (рекомендуется):
   ```
   /madspec.security
   ```
...
scripts/                  # Скрипты для определения ветки и других задач
├── bash/
│   └── get-branch.sh    # Скрипт определения текущей ветки (bash/sh)
└── powershell/
    └── get-branch.ps1   # Скрипт определения текущей ветки (PowerShell)
│   ├── concept-template.md
...
**Примечание**: 
- Команды MADSpec находятся в `.ххх/commands/` (не в `.madspec/`), так как они используются такими инструментами, как Cursor, OpenCode или SourceCraft для обработки команд.
- Ветка определяется автоматически через скрипты `scripts/bash/get-branch.sh` или `scripts/powershell/get-branch.ps1`, на которые ссылаются команды в секции `scripts`
- Скрипты выполняются из корня проекта и возвращают имя ветки через stdout
- Для управления веткой можно использовать CLI команду: `madspec branch get/set/list`
- Для миграции существующих проектов используйте: `madspec migrate`
